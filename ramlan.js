...
async function starts() {
	const haseeb = new WAConnection()
	haseeb.version = [2, 2119, 6]
	haseeb.logger.level = 'warn'
	console.log(banner.string)
	haseeb.on('qr', () => {
		console.log(color('[', 'white'), color('!', 'red'), color(']', 'white'), color('Scan Code'))
	})
	haseeb.on('credentials-updated', () => {
		fs.writeFileSync('./bot.json', JSON.stringify(haseeb.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'Login Ke Hati Dia')
	})
	fs.existsSync('./bot.json') && haseeb.loadAuthInfo('./bot.json')
	haseeb.on('connecting', () => {
		start('2', 'Connecting...')
	})
	haseeb.on('open', () => {
		success('2', 'Connected')
	})
	await haseeb.connect({ timeoutMs: 30 * 1000 })

	haseeb.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = await haseeb.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
				try {
					ppimg = await haseeb.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
...
${a}Please read our group role.If you break we will kick you from our group!${a}`
				...
				haseeb.sendMessage(mdata.id, buffer, MessageType.image, { caption: teks, contextInfo: { "mentionedJid": [num] } })
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await haseeb.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
...
${a}Your services will bury me deep inside${a}`
	...
	haseeb.on('CB:Blocklist', json => {
		if (blocked.length > 2) return
		for (let i of json[1].blocklist) {
			blocked.push(i.replace('c.us', 's.whatsapp.net'))
		}
	})
	haseeb.on('message-new', async (Lan) => {
		try {
			...
			const q = args.join(' ')
			const botNumber = haseeb.user.jid
			const totalchat = await haseeb.chats.all()
			const sender = isGroup ? Lan.participant : Lan.key.remoteJid
			pushname = haseeb.contacts[sender] != undefined ? haseeb.contacts[sender].vname || haseeb.contacts[sender].notify : undefined
			const groupMetadata = isGroup ? await haseeb.groupMetadata(from) : ''
			...
			const reply = (teks) => {
				haseeb.sendMessage(from, teks, text, { quoted: Lan })
			}
			const sendMess = (hehe, teks) => {
				haseeb.sendMessage(hehe, teks, text)
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? haseeb.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : haseeb.sendMessage(from, teks.trim(), extendedText, { quoted: Lan, contextInfo: { "mentionedJid": memberr } })
			}
			const sendImage = (teks) => {
				haseeb.sendMessage(from, teks, image, { quoted: Lan })
			}
			const costum = (pesan, tipe, target, target2) => {
				haseeb.sendMessage(from, pesan, tipe, { quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` } } })
			}
			const sendPtt = (teks) => {
				haseeb.sendMessage(from, audio, mp3, { quoted: Lan })
			}
        const fakestatus = (teks) => {
            haseeb.sendMessage(from, teks, text, {
                quoted: {
                    key: {
                        fromMe: false,
			...
			const checkLimit = (sender) => {
				...
				for (let lmt of _limit) {
					if (lmt.id === sender) {
						...
						if (limitCounts <= 0) return haseeb.sendMessage(from, `Limit Anda Sudah Habis\nUpgrade Premium Biar Bebas Limit Kak`, text, { quoted: Lan })
						haseeb.sendMessage(from, nad.limitcount(isPrem, limitCounts), text, { quoted: Lan })
						found = true
					}
				}
				if (found === false) {
			...
			const isLimit = (sender) => {
				...
				for (let i of _limit) {
					if (i.id === sender) {
						...
						if (limits >= limitawal) {
							position = true
							haseeb.sendMessage(from, nad.limitend(pushname, prefix), text, { quoted: Lan })
							return true
						} else {
							_limit
							position = true
			...
			if (isGroup) {
				try {
					...
					if (getmemex <= memberlimit) {
						...
						setTimeout(() => {
							haseeb.groupLeave(from)
						}, 5000)
						setTimeout(() => {
							haseeb.updatePresence(from, Presence.composing)
							reply("See you kak")
						}, 4000)
						setTimeout(() => {
							haseeb.updatePresence(from, Presence.composing)
				...
				setTimeout(() => {
					haseeb.groupRemove(from, [kic]).catch((e) => { reply(`BOT MUST BE ADMIN`) })
				}, 3000)
				setTimeout(() => {
					haseeb.updatePresence(from, Presence.composing)
					reply("Babay")
...
━━ 「 *BOT WHATSAPP* 」 ━━`
				haseeb.sendMessage(from, gmenu, image, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "mimetype": "image/jpeg",  "caption": cr,  "jpegThumbnail": fs.readFileSync(`./src/image/thumbnail.jpeg`) } } }, caption: menunya })
				break
				case 'owner':
				case 'creator':
					haseeb.sendMessage(from, { displayname: "Jeff", vcard: vcard }, MessageType.contact, { quoted: Lan })
					haseeb.sendMessage(from, 'Thats my Boyfriend number >_<, Ehh my owner number same', MessageType.text, { quoted: Lan })
					break

				case 'donasi':
				case 'donate':
				haseeb.sendMessage(from, nad.donasi(), text, { quoted: Lan })
					break
				case 'iklan':
				haseeb.sendMessage(from, nad.iklan(botName, ownerNumbers, ownerName), text, { quoted: Lan })
					break
...
${a}❏ Name : ${haseeb.user.name}${a}
${a}❏ Browser : ${haseeb.browserDescription[1]}${a}
${a}❏ Server : ${haseeb.browserDescription[0]}${a}
${a}❏ Version : ${haseeb.browserDescription[2]}${a}
${a}❏ Speed : ${latensii.toFixed(4)} Second${a}
${a}❏ Handphone : ${haseeb.user.phone.device_manufacturer}${a}
${a}❏ Versi WA : ${haseeb.user.phone.wa_version}${a}
...
「 *BOT WHATSAPP* 」`
				haseeb.sendMessage(from, ginfo, image, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "mimetype": "image/jpeg",  "caption": cr,  "jpegThumbnail": fs.readFileSync(`./src/image/thumbnail.jpeg`) } } }, caption: inponya })
				break
...
「 *${botName}* 」`
				...
				case 's':
					...
					if ((isMedia && !Lan.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						const media = await haseeb.downloadAndSaveMediaMessage(encmedia)
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								haseeb.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: Lan})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
				...
				case 'nuliskiri':
					...
					haseeb.sendMessage(from, kir, image, { quoted: Lan, caption: 'Nihh kak' })
					break
				case 'nuliskanan':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
					...
					haseeb.sendMessage(from, kan, image, { quoted: Lan, caption: 'Nihh kak' })
					break
				case 'stalkig':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
...
◯ Biografi : ${abu.biography}`
					haseeb.sendMessage(from, stig, image, { quoted: Lan, caption: hasil })
					break
				...
				case 'tts':
					...
					if (isLimit(sender)) return reply(nad.limitend(pusname, prefix))
					await limitAdd(sender)
					if (args.length < 1) return haseeb.sendMessage(from, `Where is the language code? Example : ${prefix}tts id Hello Haseeb`, text, { quoted: Lan })
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return haseeb.sendMessage(from, `Where is the text bro | Example : ${prefix}tts id Please wait`, text, { quoted: Lan })
					...
					bby.length > 300
						...
						: gtts.save(ranm, bby, function () {
							exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
								...
								haseeb.sendMessage(from, buff, audio, { quoted: Lan, ptt: true })
								fs.unlinkSync(rano)
							})
						})
					break
						...
						.then(async res => {
							...
							base64Img.img(ttptxt.base64, 'temp', 'ttp', function (err, filepath) {
								...
								exec(`ffmpeg -i ${pngttp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${webpng}`, (err) => {
									...
									haseeb.sendMessage(from, buffer, sticker)
				...
				case 'attp':
					...
					haseeb.sendMessage(from, atetepe, sticker, { quoted: Lan })
					break
				...
				case 'bikinquote':
					...
					haseeb.sendMessage(from, biquote, image, { caption: 'Nih kak >_<', quoted: Lan })
					break
				case 'groupmenu':
				case 'grupmenu':
					if (isBanned) return reply(nad.baned())
...
「 *${botName}* 」`
				...
				case 'antibadword':
					...
					if (Number(args[0]) === 1) {
						...
						haseeb.sendMessage(from, `ALLERT!!! Group hase been installed anti Badword\nif you break then i will kick you`, text)
				...
				case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						haseeb.sendMessage(from, `ALLERT!!! Group has been installed anti link\nIf you break i will kick you`, text)
					} else if (Number(args[0]) === 0) {
				...
				case 'group':
					...
					if (args[0] === 'open') {
						...
						haseeb.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'close') {
						reply(`Group Close by Admin`)
						haseeb.groupSettingChange(from, GroupSettingChange.messageSend, true)
					}
					break
				...
				case 'kick':
					...
					await haseeb.groupRemove(from, [kicknya])
					break
					...
					var group = await haseeb.groupMetadata(from)
					...
					haseeb.sendMessage(from, options, text)
					break
				case 'hidetag20':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
					...
					haseeb.sendMessage(from, options, text)
	                .then(() => {haseeb.sendMessage(from, options, text)})
	                .then(() => {haseeb.sendMessage(from, options, text)})
	                .then(() => {haseeb.sendMessage(from, options, text)})
	                .then(() => {haseeb.sendMessage(from, options, text)})
	                .then(() => {haseeb.sendMessage(from, options, text)})
				...
				case 'level':
					...
					haseeb.sendMessage(from, resul, text, { quoted: Lan })
						.catch(async (err) => {
							console.error(err)
							await reply(`Error!\n${err}`)
						})
				...
				case 'linkgroup':
					...
					if (!isGroup) return reply(nad.groupo())
					if (!isBotGroupAdmins) return reply(nad.badmin())
					linkgc = await haseeb.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
					haseeb.sendMessage(from, yeh, text, { quoted: Lan })
					break
				...
				case 'setname':
					...
					if (!isGroupAdmins) return reply(nad.admin())
					if (!isBotGroupAdmins) return reply(nad.badmin())
					haseeb.groupUpdateSubject(from, `${body.slice(9)}`)
					haseeb.sendMessage(from, '「 SUCCESS 」Change group name', text, { quoted: Lan })
					break
				...
				case 'setdesc':
					...
					if (!isGroupAdmins) return reply(nad.admin())
					if (!isBotGroupAdmins) return reply(nad.badmin())
					haseeb.groupUpdateDescription(from, `${body.slice(9)}`)
					haseeb.sendMessage(from, '*「 SUCCESS 」Change the gropu desk', text, { quoted: Lan })
					break
				...
				case 'demot':
					...
					if (mentioned.length > 1) {
						...
						haseeb.groupDemoteAdmin(from, mentioned)
					} else {
				...
				case 'promot':
					...
					if (mentioned.length > 1) {
						...
						haseeb.groupMakeAdmin(from, mentioned)
					} else {
				...
				case 'hedsot':
					...
					if (mentioned.length > 1) {
						...
						haseeb.groupRemove(from, mentioned)
						mentions(heds, mentioned, true)
						haseeb.groupAdd(from, [num])
					} else {
				...
				case 'fitnah':
					...
					var bot = gh.split("&")[2];
					haseeb.sendMessage(from, `${bot}`, text, { quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` } } })
					break
				...
				case 'delete':
					...
					if (isLimit(sender)) return reply(nad.limitend(pusname, prefix))
					await limitAdd(sender)
					haseeb.deleteMessage(from, { id: Lan.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
...
[WAIT] Proses Dumlu Yakan`
					...
					haseeb.sendMessage(from, pla, image, { quoted: Lan, caption: infomp3 })
					haseeb.sendMessage(from, play, audio, { mimetype: 'audio/mp4', quoted: Lan })
					break
...
[WAIT] Proses Dumlu Yakan`
					...
					haseeb.sendMessage(from, buff, image, { quoted: Lan, caption: ingfomp3 })
					haseeb.sendMessage(from, lamgu, audio, { mimetype: 'audio/mp4', quoted: Lan })
					break
...
[WAIT] Proses Dumlu Yakan`
					...
					haseeb.sendMessage(from, buffe, image, { quoted: Lan, caption: infomp4 })
					vidio = await getBuffer(anu.url)
					haseeb.sendMessage(from, vidio, video, { mimetype: 'video/mp4', quoted: Lan })
					break
				...
				case 'tiktod':
					...
					haseeb.sendMessage(from, tik, video, { mimetype: 'video/mp4', quoted: Lan })
					break
				case 'igphoto':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
					...
					haseeb.sendMessage(from, buff, image, { quoted: Lan })
					break
				...
				case 'igvideo':
					...
					haseeb.sendMessage(from, buffe, video, { mimetype: 'video/mp4', quoted: Lan })
					break
				case 'joox':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
...
[WAIT] Proses Dumlu Yakan`
					...
					haseeb.sendMessage(from, buft, image, { quoted: Lan, caption: infojoox })
					haseeb.sendMessage(from, lakgu, audio, { mimetype: 'audio/mp4', quoted: Lan })
					break
				case 'makermenu':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
...
「 *${botName}* 」`
				...
				case 'comictext':
					...
					haseeb.sendMessage(from, vhbuff, image, { quoted: Lan })
					break
				case 'hekerlogo':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
...
「 *${botName}* 」`
                    ...
                    haseeb.sendMessage(from, menghayu, image, { quoted: Lan })
                    break
                    case 'sertibabu':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertibab, image, { quoted: Lan })
                    break
                    case 'sertibucin':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertibuci, image, { quoted: Lan })
                    break
                    case 'sertibocilff':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertibocilf, image, { quoted: Lan })
                    break
                    case 'sertigay':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertiga, image, { quoted: Lan })
                    break
                    case 'sertipacar':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertipaca, image, { quoted: Lan })
                    break
                    case 'sertisadboy':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertisadbo, image, { quoted: Lan })
                    break
                    case 'sertisurga':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertisurg, image, { quoted: Lan })
                    break
                    case 'sertipinter':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertipinte, image, { quoted: Lan })
                    break
                    case 'sertibadboy':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertibadbo, image, { quoted: Lan })
                    break
                    case 'sertibadgirl':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertibadgir, image, { quoted: Lan })
                    break
                    case 'sertigoodgirl':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertigoodgir, image, { quoted: Lan })
                    break
                    case 'sertigoodboy':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertigoodbo, image, { quoted: Lan })
                    break
                    case 'sertieditor':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertiedito, image, { quoted: Lan })
                    break
                    case 'sertigudluking':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertigudlukin, image, { quoted: Lan })
                    break
                    case 'sertipakboy':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertipakbo, image, { quoted: Lan })
                    break
                    case 'sertijamet':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertijame, image, { quoted: Lan })
                    break
                    case 'sertiyutub':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertiyutu, image, { quoted: Lan })
                    break
                    case 'sertiheker':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertiheke, image, { quoted: Lan })
                    break
                    case 'sertiff1':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertiff, image, { quoted: Lan })
                    break
                    case 'sertiff2':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertif, image, { quoted: Lan })
                    break
                    case 'sertiff3':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertifa, image, { quoted: Lan })
                    break
                    case 'sertiff4':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertifb, image, { quoted: Lan })
                    break
                    case 'sertiff5':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertifc, image, { quoted: Lan })
                    break
                    case 'sertipubg1':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertipubg, image, { quoted: Lan })
                    break
                    case 'sertipubg2':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertipub, image, { quoted: Lan })
                    break
                    case 'sertiml':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
                    ...
                    haseeb.sendMessage(from, sertim, image, { quoted: Lan })
                    break
				case 'gabutmenu':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
...
「 *${botName}* 」`
				...
				case 'tebakin':
					...
					setTimeout(() => {
						haseeb.sendMessage(from, '*➸ Jawaban :* ' + anu.result.jawaban, text, { quoted: Lan })
					}, 30000) // 1000 = 1s,
					setTimeout(() => {
						haseeb.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout(() => {
						haseeb.sendMessage(from, '_20 Detik lagi_…', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout(() => {
						haseeb.sendMessage(from, '_30 Detik lagi_…', text) // ur cods
					}, 2500) // 1000 = 1s,
					setTimeout(() => {
						haseeb.sendMessage(from, tebak, image, { caption: '_Jawab Ye, Gak Bisa Jawab\nHarus Donasi_', quoted: Lan })
					}, 0) // 1000 = 1s,
					break
				case 'caklontong':
					if (isBanned) return reply(nad.baned())
					...
					setTimeout(() => {
						haseeb.sendMessage(from, '*➸ Jawaban :* ' + anu.result.jawaban + '\n' + anu.result.desk, text, { quoted: Lan })
					}, 30000) // 1000 = 1s,
					setTimeout(() => {
						haseeb.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					...
					setTimeout(() => {
						haseeb.sendMessage(from, anu.result.soal, text, { quoted: Lan })
					}, 0) // 1000 = 1s,
					break
				...
				case 'bisakah':
					...
					haseeb.sendMessage(from, 'Pertanyaan : *' + bisakah + '*\n\nJawaban : ' + keh, text, { quoted: Lan })
					break
					...
					haseeb.sendMessage(from, 'Pertanyaan : *' + kapankah + '*\n\nJawaban : ' + koh, text, { quoted: Lan })
					break
				...
				case 'apakah':
					...
					haseeb.sendMessage(from, 'Pertanyaan : *' + apakah + '*\n\nJawaban : ' + kah, text, { quoted: Lan })
					break
				...
				case 'rate':
					...
					haseeb.sendMessage(from, 'Pertanyaan : *' + rate + '*\n\nJawaban : ' + te + '%', text, { quoted: Lan })
					break
				...
				case 'hobby':
					...
					haseeb.sendMessage(from, 'Pertanyaan : *' + hobby + '*\n\nJawaban : ' + by, text, { quoted: Lan })
					break
				...
				case 'truth':
					...
					haseeb.sendMessage(from, truteh, image, { caption: '*Truth*\n\n' + ttrth, quoted: Lan })
					break
				...
				case 'dare':
					...
					haseeb.sendMessage(from, tod, image, { quoted: Lan, caption: '*Dare*\n\n' + der })
					break
				...
				case 'cekbapak': // By Ramlan ID
					...
					haseeb.sendMessage(from, cek, text, { quoted: Lan })
					break
...
「 *${botName}* 」`
				...
				case 'meme':
					...
					haseeb.sendMessage(from, mimi, image, { quoted: Lan })
					break
			...
			case 'waifu':
					...
					haseeb.sendMessage(from, ifu, image, {quoted: Lan, caption: "Wibu AbiZzz"})
					})})
					} catch (e) {
			...
			case 'neko':
				    ...
				    try {
						...
						.then((response) => {
						var ifu = Buffer.from(response, 'base64');
					haseeb.sendMessage(from, ifu, image, {quoted: Lan})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('Error!')
			...
			case 'shinobu':
				...
				case 'loli':
					...
					haseeb.sendMessage(from, lomli, image, { quoted: Lan, caption: 'Cintai Loli Mu>_<' })
					break
				...
				case 'nekonime':
					...
					haseeb.sendMessage(from, neko, image, { quoted: Lan, caption: 'Nekonime >_<' })
					break
				...
				case 'sagiri':
					...
					reply(nad.wait())
					haseeb.sendMessage(from, sagi, image, { quoted: Lan })
					break
				case 'estetik':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
					...
					haseeb.sendMessage(from, este, image, { quoted: Lan })
				break
...
「 *${botName}* 」`
				...
				case 'tomp3':
					...
					media = await haseeb.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('try again:)')
						mhee = fs.readFileSync(ran)
						haseeb.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', quoted: Lan })
						fs.unlinkSync(ran)
					})
					break
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						haseeb.sendMessage(from, buffer, image, { quoted: Lan, caption: 'nih kak [(^.^)]' })
						fs.unlinkSync(ran)
					})
					break
                ...
                case 'tomp4':
                    ...
                    if ((isMedia && !Lan.message.videoMessage || isQuotedSticker) && args.length == 0) {
                        ...
                        owgi = await haseeb.downloadAndSaveMediaMessage(ger)
                            ...
                            .then(({ data }) => {
                                ...
                                axios({
                                    ...
                                    getBuffer(result).then(tog => {
                                        haseeb.sendMessage(from, tog, video, { mimetype: 'video/mp4', quoted: Lan })
                                    })
                                })
                            })
                    } else {
				...
				case 'imgtourl':
					...
					var media = await haseeb.downloadAndSaveMediaMessage(encmedia)
					var imgbb = require('imgbb-uploader')
...
➸ URL : ${data.display_url}`
							...
							haseeb.sendMessage(from, ibb, image, { quoted: Lan, caption: caps })
						})
						.catch(err => {
							throw err
						})
				...
				case 'komenyt':
					...
					var imgbb = require('imgbb-uploader')
					try {
						pp = await haseeb.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
					} catch {
					...
					haseeb.sendMessage(from, buffer, image, { caption: 'Nih Cok', contextInfo: { participant: '0@s.whatsapp.net', quotedMessage: { conversation: '*_YOUTUBE COMMENT_*' } } })
					break
				...
				case 'trigered':
					...
					if ((isMedia && !Lan.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
							...
							haseeb.sendMessage(from, nobg, sticker, { quoted: Lan })
							fs.unlinkSync(rano)
						})
					} else {
			    ...
			    case 'nightcore':
					...
					haseeb.sendMessage(from, hah, audio, { mimetype: 'audio/mp4', quoted: Lan, ptt: true })
						fs.unlinkSync(ran)
					    })
				       break
				case 'slow':
				...
				case 'ghost':
					...
					haseeb.sendMessage(from, ghs, audio, { mimetype: 'audio/mp4', quoted: Lan, ptt: true })
						fs.unlinkSync(ran)
					    })
				       break
		       case 'bass':
	             ...
	             case 'toptt':
					...
					haseeb.sendMessage(from, topt, audio, { mimetype: 'audio/mp4', quoted: Lan, ptt: true })
						})
						await limitAdd(sender)
					    break
				case 'mutualmenu':
...
➸ Org : ${lacaks.org}
➸ As : ${lacaks.as}`
					haseeb.sendMessage(from, lacak, text, { quoted: Lan })
					break
				...
				case 'brainly':
					...
					brainly(`${brien}`).then(res => {
						...
						haseeb.sendMessage(from, teks, text, { quoted: Lan, detectLinks: false })
						console.log(res)
					})
					break
				...
				case 'wiki':
					...
					haseeb.sendMessage(from, wikiped, text, { quoted: Lan })
					break
				...
				case 'kbbi':
					...
					haseeb.sendMessage(from, kabebei, text, { quoted: Lan })
					break
...
Meninggal : ${anu.result.meninggal}`
					haseeb.sendMessage(from, cvd, text, { quoted: Lan })
					break
				...
				case 'pinterest':
					...
					haseeb.sendMessage(from, pok, image, { quoted: Lan, caption: `*PINTEREST*` })
					break
					case 'jadwalsholat':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
					...
					haseeb.sendMessage(from, jsholat, text, {quoted: Lan})
					break
...
「 *${botName}* 」`
				...
				case 'addstiker':
					...
					lan = await haseeb.downloadMediaMessage(adds)
				...
				case 'getstiker':
					...
					haseeb.sendMessage(from, hasilya, sticker, { quoted: Lan })
					break
				...
				case 'liststiker':
					...
					lis += `\n╰─────「 *${setimker.length}* 」`
					haseeb.sendMessage(from, lis.trim(), extendedText, { quoted: Lan, contextInfo: { "mentionedJid": setimker } })
					break
				...
				case 'addvideo':
					...
					dvi = await haseeb.downloadMediaMessage(deo)
					vidioya.push(`${adv}`)
					fs.writeFileSync(`./media/video/${adv}.mp4`, dvi)
					fs.writeFileSync(`./media/video.json`, JSON.stringify(vidioya))
					haseeb.sendMessage(from, `Video Added in Bot Database`, MessageType.text, { quoted: Lan })
					break
				...
				case 'getvideo':
					...
					haseeb.sendMessage(from, buffer, video, { mimetype: 'video/mp4', quoted: Lan })
					break
				...
				case 'listvideo':
					...
					list += `\n╰─────「 *${vidioya.length}* 」`
					haseeb.sendMessage(from, list.trim(), extendedText, { quoted: Lan, contextInfo: { "mentionedJid": vidioya } })
					break
				...
				case 'addvn':
					...
					delb = await haseeb.downloadMediaMessage(boij)
					audioya.push(`${advn}`)
					fs.writeFileSync(`./media/audio/${advn}.mp3`, delb)
					fs.writeFileSync('./media/audio.json', JSON.stringify(audioya))
					haseeb.sendMessage(from, `Vn Added in Bot Database`, MessageType.text, { quoted: Lan })
					break
				...
				case 'getvn':
					...
					haseeb.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: Lan, ptt: true })
					break
				...
				case 'listvn':
					...
					lisv += `\n╰─────「 *${audioya.length}* 」`
					haseeb.sendMessage(from, lisv.trim(), extendedText, { quoted: Lan, contextInfo: { "mentionedJid": audioya } })
					break
				...
				case 'addimage':
					...
					imej = await haseeb.downloadMediaMessage(svimeg)
					imegya.push(`${sepimg}`)
					fs.writeFileSync(`./media/image/${sepimg}.jpeg`, imej)
					fs.writeFileSync('./media/image.json', JSON.stringify(imegya))
					haseeb.sendMessage(from, `Image added in Bot Database`, MessageType.text, { quoted: Lan })
					break
				...
				case 'getimage':
					...
					haseeb.sendMessage(from, buffer, image, { quoted: Lan })
					break
				...
				case 'listimage':
					...
					lisi += `\n╰─────「 *${imegya.length}* 」`
					haseeb.sendMessage(from, lisi.trim(), extendedText, { quoted: Lan, contextInfo: { "mentionedJid": imegya } })
					break
				case 'iri':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
					...
					haseeb.sendMessage(from, irim, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'pale':
					...
					haseeb.sendMessage(from, pal, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'pota':
					...
					haseeb.sendMessage(from, pot, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'welot':
					...
					haseeb.sendMessage(from, wel, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'alay':
					...
					haseeb.sendMessage(from, ala, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'bernyanyi':
					...
					haseeb.sendMessage(from, ber, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'bwa':
					...
					haseeb.sendMessage(from, bw, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'ganteng':
					...
					haseeb.sendMessage(from, gan, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'gatal':
					...
					haseeb.sendMessage(from, ga, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'ladida':
					...
					haseeb.sendMessage(from, lada, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'rusher':
					...
					haseeb.sendMessage(from, rus, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'boong':
					...
					haseeb.sendMessage(from, boo, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'tengteng':
					...
					haseeb.sendMessage(from, teng, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'sound1':
					...
					haseeb.sendMessage(from, satu, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'sound2':
					...
					haseeb.sendMessage(from, dua, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'sound3':
					...
					haseeb.sendMessage(from, tiga, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'sound4':
					...
					haseeb.sendMessage(from, empat, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'sound5':
					...
					haseeb.sendMessage(from, lima, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'sound6':
					...
					haseeb.sendMessage(from, enam, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				...
				case 'sound7':
					...
					haseeb.sendMessage(from, tujuh, MessageType.audio, { quoted: Lan, mimetype: 'audio/mp4', ptt: true })
					break
				case 'ownermenu':
...
「 *${botName}* 」`
                ...
                if (!isOwner) return reply(nad.ownerb())
                    ...
                    const downiamgethumb = await haseeb.downloadMediaMessage(messimagethumb)
				...
				if (!isQuotedImage) return reply(`Send image with caption ${prefix}setbotpp or tag already send image`)
					...
					media = await haseeb.downloadAndSaveMediaMessage(enmedia)
					await haseeb.updateProfilePicture(botNumber, media)
					reply('Profile picture change successfully😗')
					break
                 case 'readall':
					if (!isOwner) return reply(nad.ownerb())
					var chats = await haseeb.chats.all()
                    chats.map( async ({ jid }) => {
                          await haseeb.chatRead(jid)
                    })
					rdl = `${a}Berhasil membaca ${chats.length} Chat !${a}`
					await haseeb.sendMessage(from, rdl, MessageType.text, {quoted: Lan})
				...
				haseeb.sendMessage(from, pemlist.trim(), extendedText, {quoted: Lan, contextInfo: {"mentionedJid": premium}})
				break
				...
				case 'bc':
					...
					anu = await haseeb.chats.all()
					if (isMedia && !Lan.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(Lan).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : Lan
						buff = await haseeb.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							haseeb.sendMessage(_.jid, buff, image, { caption: `*「 ${botName} BROADCAST 」*\n\n${body.slice(4)}` })
						}
						reply('')
					} else {
						for (let _ of anu) {
				...
				case 'clearall':
					if (!isOwner) return reply(nad.ownerb())
					anu = await haseeb.chats.all()
					haseeb.setMaxListeners(25)
					for (let _ of anu) {
						haseeb.deleteChat(_.jid)
					}
				...
				case 'term':
					...
					exec(cmd, (err, stdout) => {
						if (err) return haseeb.sendMessage(from, `root@Ramlan:~ ${err}`, text, { quoted: Lan })
						if (stdout) {
							haseeb.sendMessage(from, stdout, text, term)
						}
					})
					break

				case 'return':
					return haseeb.sendMessage(from, JSON.stringify(eval(args.join(''))), text, { quoted: Lan })
					break
				default:
					if (budy == '@verify') {
						if (isBanned) return reply(nad.baned())
						...
						if (isGroup) {
							...
							try {
								ppadd = await haseeb.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
							} catch {
...
╰─────「 *${botName}* 」`
							...
							haseeb.sendMessage(from, peripi, image, {
...
╰─────「 *${botName}* 」`
			...
			if (isGroup && !isCmd && budy != undefined) {
				console.log(budy)
				//		reply(haseeb.cmdnf(prefix, command))
			} else {
